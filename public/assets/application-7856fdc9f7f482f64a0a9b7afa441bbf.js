...
(function() {
  ...
  user = {
    setupForm: function() {
      return $('#user_plan').submit(function() {
    ...
    handleStripeResponse: function(status, response) {
      if (status === 200) {
        ...
        return $('#user_plan')[0].submit();
      } else {
        $('#stripe_error').text(response.error.message);
        $('input[type=submit]').attr('disabled', false);
        $('.setup_billing_button').show();
        return $('.subscription_button_gif').hide();
      }
    }
  };
...
(function() {
  ...
  user = {
    setupForm: function() {
      return $('#update_plan').submit(function() {
        $('input[type=submit]').attr('disabled', true);
        if ($('#card_number').length) {
          user.processCard();
          return false;
